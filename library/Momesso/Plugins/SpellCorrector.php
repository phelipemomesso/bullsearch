...
class Momesso_Plugins_SpellCorrector extends Zend_Controller_Plugin_Abstract {
	...
	private static $NWORDS;
		...
		public static function correct($word) {
			...
			if(empty(self::$NWORDS)) {
				...
				$path_dict = getcwd() . '/default/uploads/spell/';
				
				if(!file_exists($path_dict.'serialized_dictionary.txt')) {
					self::$NWORDS = self::train(self::words(file_get_contents($path_dict."dictionary.txt")));
					$fp = fopen($path_dict."serialized_dictionary.txt","w+");
					fwrite($fp,serialize(self::$NWORDS));
					fclose($fp);
				} else {
					self::$NWORDS = unserialize(file_get_contents($path_dict."serialized_dictionary.txt"));
				}
			}
			$candidates = array(); 

...
class Momesso_Admin_Form_Touros_Goal extends EasyBib_Form {
	...
	public function init() {
        ...
        $Row = $Country->getCountries();
        $Dados = array();
        $Dados[''] = '-- Select --';
        foreach ($Row as $v) {
            $Dados[$v['cod_country']] = stripslashes($v['name']);
        }

        $country = $this->createElement('select', 'country', array('label' => 'Country: '));
        $country->setRequired(true)->setMultiOptions($Dados);
